...
const STORAGE_KEYS = {
  ...
  INSTAGRAM_ID: "instagramBusinessId", // Store Instagram Business Account ID
...
export default function InstagramCard() {
  const [connection, setConnection] = useState(() => ({
    facebookPageId:
      typeof window !== "undefined"
        ? localStorage.getItem(STORAGE_KEYS.FACEBOOK_PAGE_ID)
        : null,
    instagramId:
      typeof window !== "undefined"
  ...
  const checkConnection = useCallback(async (facebookPageId) => {
    try {
  ...
  useEffect(() => {
    const validateConnection = async () => {
      ...
      if (facebookPageId && (!lastValidated || Date.now() - lastValidated > VALIDATION_INTERVAL)) {
        await checkConnection(facebookPageId);
      }
    };
  ...
  }, [connection.instagramId, connectInstagram]);
  ...
  return (
    <div className="bg-white rounded-md p-8 grid gap-8 w-[25rem]">
      ...
      <div className="flex justify-end items-center">
        <Switch checked={!!connection.instagramId} onCheckedChange={handleToggle} />
      </div>
    </div>
  );
}
